		...
		<div className={`relative ${className}`}>
			...
			>
				...
				{planePositions.map((plane) => (
					// biome-ignore lint/a11y/noStaticElementInteractions: This is a custom interactive element that requires click and keyboard handling for accessibility.
					<g
						key={plane.icao24}
						transform={`translate(${plane.x}, ${plane.y})`}
						className={`cursor-pointer ${plane.isSelected ? "glow-pulse" : plane.isOverhead ? "glow-pulse" : ""}`}
						onClick={() =>
							onSelectFlight(plane.isSelected ? null : plane.icao24)
						}
						tabIndex={0}
						onKeyDown={(e) => {
					...
					>
						...
						<g transform={`rotate(${plane.heading}, 0, 0)`}>
							<path
								...
								className={
									plane.isOverhead || plane.isSelected ? "glow-pulse" : ""
								}
							/>
						</g>
						{/* Callsign label */}
