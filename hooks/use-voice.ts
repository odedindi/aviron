...
import { getAirlineName, metersToFeet, getHeadingDirection, VOICE_TRANSLATIONS, type VoiceSettings, type FlightData } from '@/lib/types';
...
export function useVoice(voiceSettings: VoiceSettings) {
  ...
  useEffect(() => {
    ...
    if (wasMuted && !isMuted) {
      ...
      if (timeSinceMute < 30000 && pendingAnnouncements.current.length > 0) {
        ...
        const recentFlight = pendingAnnouncements.current[pendingAnnouncements.current.length - 1];
        pendingAnnouncements.current = [];
  ...
  const getVoice = useCallback((): SpeechSynthesisVoice | null => {
    ...
    if (voiceSettings.voiceId) {
      const index = availableVoices.findIndex(v => v.id === voiceSettings.voiceId);
  ...
  const getVoicesForLanguage = useCallback((langCode: string): VoiceOption[] => {
  ...
  const buildAnnouncementText = useCallback((flight: FlightData): string => {
    ...
    const translations = VOICE_TRANSLATIONS[voiceSettings.language] || VOICE_TRANSLATIONS['en-US'];
  ...
  const announceInternal = useCallback((text: string, force: boolean = false) => {
  ...
  const announceFlightInternal = useCallback((flight: FlightData) => {
    const text = buildAnnouncementText(flight);
    announceInternal(text, false);
  }, [buildAnnouncementText, announceInternal]);

  // Announce overhead flight
  const announceOverheadFlight = useCallback((flight: FlightData) => {
    const callsign = flight.callsign?.trim() || flight.icao24;
