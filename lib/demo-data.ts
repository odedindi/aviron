...
export function generateDemoFlights(
	...
	for (let i = 0; i < numFlights; i++) {
		...
		flights.push({
			...
			callsign:
				DEMO_CALLSIGNS[Math.floor(Math.random() * DEMO_CALLSIGNS.length)],
			originCountry: "Demo",
...
export function getDemoRoutes(): Record<
	string,
...
> {
	...
	DEMO_CALLSIGNS.forEach((callsign) => {
		const origin =
			DEMO_ORIGINS[Math.floor(Math.random() * DEMO_ORIGINS.length)];
		let destination =
